//Name: Go Go Gorelians
//Level: 3
//Category: グラフ,Graph,幅優先探索,BFS,経路復元
//Note:

/*
 * 2回のBFSで最遠点対が求められるので，その中間点を出力する．
 */
#include <iostream>
#include <vector>
#include <queue>
...
struct Planet {
    ...
    int dist2(const Planet &other) const {
    ...
    while(true) {
        ...
        }
        {
            ...
            for(int i = 0; i < maxcost; ++i) {
                if(i == maxcost/2) {
                    ans.push_back(v[cur].id);
                }
                if(maxcost%2 == 1 && i == maxcost/2+1) {
                    ans.push_back(v[cur].id);
                }
                cur = prev[cur];
            }
